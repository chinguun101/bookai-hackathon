...
if prompt := st.chat_input("Enter your prompt here..."):
    ...
    if uploaded_file:
        ...
        download_link = f'<a href="data:text/plain;base64,{uploaded_file_base64}" download="{uploaded_file.name}" target="_blank">Open</a>'
    else:
        download_link = ""
    for idx, excerpt in enumerate(unique_excerpts):
        excerpt_with_button = f"""
        <div style="position:relative;">
            <div style="float:left; width:90%;">{excerpt}</div>
        ...
        <hr>
        """
        st.markdown(excerpt_with_button, unsafe_allow_html=True)

    separator = "\n\n------------------------------------------------------\n\n"
    combined_prompt = f"User Question: {prompt}\n\nRelevant Excerpt(s):\n\n{separator.join(unique_excerpts)}"
